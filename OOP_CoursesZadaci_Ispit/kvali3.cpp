...
private:
    int goleminaTim;
public:
    TeamProject(string name = "Project1", int duration = 2,int goleminaTim=0) : Project (name,duration){
        if(goleminaTim>0 && goleminaTim<10){
            this->goleminaTim=goleminaTim;
        }else{
            this->goleminaTim=5;
        }
    }
    ...
    int calculateTotalTeamHours(){
        return duration+ (goleminaTim*160);
    }

    void showInfo(){
        cout<<name<<" "<<duration<<" "<<goleminaTim<<endl;
    }
};
