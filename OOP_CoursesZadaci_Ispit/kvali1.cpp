...
private:
    int bonusKrediti;
public:
    OnlineCourse(string code = "CS101", int credits = 6,int bonusKrediti=1) : Course(code,credits) {
        if(bonusKrediti>0 && bonusKrediti<3){
            this->bonusKrediti = bonusKrediti;
        }else{
            this->bonusKrediti=1;
        }
    ...
    int calculateMaxCourseCredits(){
        return credits+bonusKrediti;
    }

    void display(){
        cout<<code<<" "<<credits<<" "<<bonusKrediti<<endl;
    }
};
